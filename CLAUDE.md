...
1. **`build-imagebuilder.yml`** — `discover` job lists directories in [pesa1234/MT6000_cust_build](https://github.com/pesa1234/MT6000_cust_build) (named `{date}_{revision-hash}_{branch}`), picks the latest, checks whether the corresponding pesa1234/openwrt commit SHA was already built (stored in our GH release body). `build` job compiles OpenWrt + ImageBuilder with pesa's recommended `.config` plus `CONFIG_IB=y` (ours also adds `CONFIG_IB_STANDALONE=y`). `containerize` job pushes the slim runtime image to GHCR and publishes packages + metadata to the `releases` branch.
...
The `Containerfile` in this repo is **stage 2** (slim runtime). The tarball it expects is produced by the `build` job in `build-imagebuilder.yml` and copied into the build context before `docker build` runs.
